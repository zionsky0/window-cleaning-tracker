...
import { Sparkles, Plus, Cloud, FileSpreadsheet, CheckCircle2, Banknote, CreditCard, Clock, Landmark } from 'lucide-react';
...
export function Header({
  ...
  return (
    <>
      ...
      {/* Daily Round Summary Cards (Shown on Today/Route tab - naturally scrolls with page to maximize mobile viewport) */}
      {showSummaryCards && (
        <div className="bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800/80 transition-colors duration-200">
          ...
          <div className="px-4 pb-2.5 pt-0.5 grid grid-cols-4 gap-1.5 sm:gap-2">
            ...
            <div className="bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200/70 dark:border-indigo-900/40 rounded-xl p-2 flex flex-col justify-between">
              <span className="text-[10px] font-bold text-indigo-800 dark:text-indigo-300 flex items-center gap-1 truncate">
                <Landmark className="w-3 h-3 text-indigo-600 dark:text-indigo-400 shrink-0" />
                <span className="truncate">BACS</span>
              </span>
              <div className="mt-0.5 flex items-baseline justify-between">
                <span className="text-sm font-black text-indigo-800 dark:text-indigo-300">
                  £{stats.bacsAmount}
                </span>
