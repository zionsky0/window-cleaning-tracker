...
import { X, Cloud, Phone, Mail, Lock, Check, LogOut, RefreshCw, AlertCircle, Sparkles } from 'lucide-react';
...
export function SyncModal({
  ...
  const handleSync = async (e: React.FormEvent) => {
    ...
    if (!identifier || !cleanPin) {
      setMessage({ text: 'Please enter both your identifier and PIN/password.', type: 'error' });
      return;
    }
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Access your rounds on any phone or tablet</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            <form onSubmit={handleSync} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-slate-700 block mb-1 flex items-center gap-1">
                  ...
                  {tab === 'phone' ? '4-Digit Secret PIN' : 'Password'}
                </label>
                <input
                  type={tab === 'phone' ? 'tel' : 'password'}
                  maxLength={tab === 'phone' ? 4 : undefined}
                  required
                  placeholder={tab === 'phone' ? 'e.g. 1234' : '••••••••'}
                  value={pin}
                  onChange={(e) => setPin(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm font-mono text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                />
                <p className="text-[11px] text-slate-400 mt-1">
                  If this is your first time, this creates your account. If returning, this logs you back in.
                </p>
              </div>
